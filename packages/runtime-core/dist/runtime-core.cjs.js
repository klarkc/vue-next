...
function emit(instance, event, ...rawArgs) {
  ...
  }
  {
    ...
    if (
      ...
      props[shared.toHandlerKey(lowerCaseEvent)]
    ) {
      warn(
        `Event "${lowerCaseEvent}" is emitted in component ` +
          ...
          `You should probably use "${shared.hyphenate(
            event
          )}" instead of "${event}".`
      )
    }
...
function isEmitListener(options, key) {
  ...
  return (
    shared.hasOwn(options, key[0].toLowerCase() + key.slice(1)) ||
    ...
    for (const key in rawCurrentProps) {
      if (
        ...
        (!shared.hasOwn(rawProps, key) &&
          ...
          ((kebabKey = shared.hyphenate(key)) === key ||
            !shared.hasOwn(rawProps, kebabKey)))
      ) {
        if (options) {
          if (
            rawPrevProps &&
...
const normalizeSlotValue = value =>
  shared.isArray(value) ? value.map(normalizeVNode) : [normalizeVNode(value)]
...
*/
const isBuiltInDirective = /*#__PURE__*/ shared.makeMap(
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const unmount = (
    ...
    if (shapeFlag & 6 /* COMPONENT */) {
      ...
      if (shapeFlag & 64 /* TELEPORT */) {
        vnode.type.remove(vnode, internals, doRemove)
      }
      if (doRemove) {
        remove(vnode)
      }
...
function resolveAsset(type, name, warnMissing = true) {
  ...
  if (instance) {
    ...
    warn(
      `resolve${shared.capitalize(type.slice(0, -1))} ` +
...
function cloneVNode(vnode, extraProps, mergeRef = false) {
  ...
  return {
    ...
    children:
      patchFlag === -1 /* HOISTED */ && shared.isArray(children)
...
function applyOptions(
  ...
  if (asMixin) {
    if (components) {
      shared.extend(
        instance.components ||
          (instance.components = shared.extend({}, instance.type.components)),
        components
      )
    }
    if (directives) {
      shared.extend(
        instance.directives ||
          (instance.directives = shared.extend({}, instance.type.directives)),
        directives
      )
    }
  }
  ...
  if (shared.isArray(expose)) {
    if (!asMixin) {
      if (expose.length) {
        const exposed =
          instance.exposed || (instance.exposed = reactivity.proxyRefs({}))
        expose.forEach(key => {
...
function initCustomFormatter() {
  ...
  function formatInstance(instance) {
    ...
    if (instance.type.props && instance.props) {
      blocks.push(
        createInstanceBlock('props', reactivity.toRaw(instance.props))
      )
    }
  ...
  function isKeyOfType(Comp, key, type) {
    ...
    if (
      (shared.isArray(opts) && opts.includes(key)) ||
