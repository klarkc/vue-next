...
const isBuiltInType = (tag, expected) =>
  tag === expected || tag === shared.hyphenate(expected)
...
function createRootCodegen(root, context) {
  ...
  if (children.length === 1) {
    ...
    if (children.filter(c => c.type !== 3 /* COMMENT */).length === 1) {
      ...
      patchFlagText += `, ${
        shared.PatchFlagNames[2048 /* DEV_ROOT_FRAGMENT */]
      }`
    }
...
function genNodeListAsArray(nodes, context) {
  const multilines =
    nodes.length > 3 || nodes.some(n => shared.isArray(n) || !isText$1(n))
  context.push(`[`)
...
const isLiteralWhitelisted = /*#__PURE__*/ shared.makeMap(
...
const transformElement = (node, context) => {
  ...
  return function postTransformElement() {
    ...
    const isDynamicComponent =
      shared.isObject(vnodeTag) && vnodeTag.callee === RESOLVE_DYNAMIC_COMPONENT
    let vnodeProps
    let vnodeChildren
    let vnodePatchFlag
    let patchFlag = 0
    ...
    if (patchFlag !== 0) {
      {
        if (patchFlag < 0) {
          ...
          vnodePatchFlag =
            patchFlag + ` /* ${shared.PatchFlagNames[patchFlag]} */`
        } else {
          // bitwise flags
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        if (isText(child) || child.type === 8 /* COMPOUND_EXPRESSION */) {
          ...
          if (
            ...
            callArgs.push(
              1 /* TEXT */ + ` /* ${shared.PatchFlagNames[1 /* TEXT */]} */`
            )
          }
          children[i] = {
            type: 12 /* TEXT_CALL */,
...
function baseCompile(template, options = {}) {
  ...
  const ast = shared.isString(template)
