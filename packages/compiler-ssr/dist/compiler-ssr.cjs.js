      ...
      node.ssrCodegenNode = compilerDom.createCallExpression(
        ...
        [
          ...
          compilerDom.createCallExpression(
            context.helper(compilerDom.CREATE_VNODE),
...
] = compilerDom.getBaseTransformPreset(true)
const vnodeNodeTransforms = [
  ...baseNodeTransforms,
          ...
          if (vModel) {
            ...
            propsExp.arguments = [
              compilerDom.createSequenceExpression([
                ...
                compilerDom.createCallExpression(
                  context.helper(compilerDom.MERGE_PROPS),
                  [
                    tempExp,
                    compilerDom.createCallExpression(
    ...
    for (let i = 0; i < node.props.length; i++) {
      ...
      if (prop.type === 7 /* DIRECTIVE */) {
        if (prop.name === 'html' && prop.exp) {
          ...
          if (!hasDynamicVBind) {
            node.children = [
              compilerDom.createInterpolation(prop.exp, prop.loc)
            ]
          }
        } else {
                ...
                if (attrName === 'class') {
                  ...
                  attrName =
                    node.tag.indexOf('-') > 0
                      ...
                      : shared.propsToAttrMap[attrName] ||
                  ...
                  if (shared.isBooleanAttr(attrName)) {
                    ...
                    openTag.push(
                      compilerDom.createCallExpression(
                        context.helper(SSR_RENDER_ATTR),
                        [key, value]
                      )
                    )
                  } else {
...
function ssrProcessElement(node, context) {
  ...
  if (context.withSlotScopeId) {
    context.pushStringPart(
      compilerDom.createSimpleExpression(`_scopeId`, false)
    )
  }
  // close open tag
  context.pushStringPart(`>`)
...
function ssrCodegenTransform(ast, options) {
  ...
  if (options.ssrCssVars) {
    const varsExp = compilerDom.processExpression(
      ...
      compilerDom.createTransformContext(compilerDom.createRoot([]), options)
    )
    context.body.push(
      compilerDom.createCompoundExpression([
  ...
  const isFragment =
    ast.children.length > 1 && ast.children.some(c => !compilerDom.isText(c))
    ...
    if (node.tag === 'input') {
      ...
      if (type) {
        ...
        if (type.type === 7 /* DIRECTIVE */) {
          ...
          res.ssrTagParts = [
            compilerDom.createCallExpression(
              context.helper(SSR_RENDER_DYNAMIC_MODEL),
              [type.exp, model, value]
            )
          ]
        } else if (type.value) {
          // static type
            ...
            case 'radio':
              res.props = [
                compilerDom.createObjectProperty(
                  `checked`,
                  compilerDom.createCallExpression(
                    context.helper(SSR_LOOSE_EQUAL),
                    [model, value]
                  )
                )
              ]
              ...
              if (trueValueBinding) {
                ...
                res.props = [
                  compilerDom.createObjectProperty(
                    `checked`,
                    compilerDom.createConditionalExpression(
                      compilerDom.createCallExpression(`Array.isArray`, [
                        model
                      ]),
                      compilerDom.createCallExpression(
                        context.helper(SSR_LOOSE_CONTAIN),
                        [model, value]
                      ),
                      model
                    )
...
const ssrTransformShow = (dir, node, context) => {
  if (!dir.exp) {
    context.onError(
      compilerDom.createDOMCompilerError(57 /* X_V_SHOW_NO_EXPRESSION */)
    )
  }
  return {
    props: [
