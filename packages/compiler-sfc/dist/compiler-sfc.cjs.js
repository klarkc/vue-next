...
var selectorParser__default = /*#__PURE__*/ _interopDefaultLegacy(
  selectorParser
)
...
function genCssVarsCode(vars, bindings, id, isProd) {
  ...
  const context = CompilerDOM.createTransformContext(
    CompilerDOM.createRoot([]),
    {
  ...
  if (node.type === 1 /* ELEMENT */) {
    ...
    node.props.forEach((attr, index) => {
      ...
      if (options.base && attr.value.content[0] === '.') {
        ...
        attr.value.content =
          host +
          (path__default.posix || path__default).join(
            basePath,
            url.path + (url.hash || '')
          )
        return
    ...
    if (srcsetTags.includes(node.tag) && node.props.length) {
      node.props.forEach((attr, index) => {
        if (attr.name === 'srcset' && attr.type === 6 /* ATTRIBUTE */) {
          ...
          if (options.base) {
            ...
            imageCandidates.forEach(({ url, descriptor }) => {
              ...
              if (isRelativeUrl(url)) {
                set.push(
                  (path__default.posix || path__default).join(base, url) +
                    descriptor
                )
              } else {
          ...
          const compoundExpression = compilerCore.createCompoundExpression(
            [],
            attr.loc
          )
        ...
        if (n.nodes.length) {
          ...
          if (!prev || !isSpaceCombinator(prev)) {
            selector.insertAfter(
              n,
              selectorParser__default.combinator({
                value: ' '
              })
            )
          }
